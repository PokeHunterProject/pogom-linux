...
def get_encryption_lib_path():
    ...
    if sys.platform == "win32" or sys.platform == "cygwin":
        if platform.architecture()[0] == '64bit':
            lib_name = "niantichash64.dll"
        else:
            lib_name = "libniantichash32.dll"

    elif sys.platform == "darwin":
        lib_name = "libniantichash-osx-64.so"

    elif os.uname()[4].startswith("arm") and platform.architecture()[0] == '32bit':
        lib_name = "libniantichash-linux-arm-32.so"

    elif os.uname()[4].startswith("aarch64") and platform.architecture()[0] == '64bit':
        lib_name = "libniantichash-linux-arm-64.so"

    elif sys.platform.startswith('linux'):
        if "centos" in platform.platform():
            if platform.architecture()[0] == '64bit':
                lib_name = "libniantichash-centos-x86-64.so"
            else:
                lib_name = "libniantichash-linux-x86-32.so"
        else:
            if platform.architecture()[0] == '64bit':
                lib_name = "libniantichash-linux-x86-64.so"
            else:
                lib_name = "libniantichash-linux-x86-32.so"

    elif sys.platform.startswith('freebsd'):
        lib_name = "libniantichash-freebsd-64.so"
